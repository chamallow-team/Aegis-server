...
impl Packet {
    ...
    pub fn len(&self) -> u64 {
        return self
            ...
            .try_into()
            .expect("failed to convert length to u64.. wtf?");
    }
...
mod tests {
    ...
    fn packet_get_set_headers() {
        ...
        assert_eq!(
            it.next().expect("missing Version header"),
            &Header::Version("1.0.3".to_string())
        );
        assert_eq!(
            it.next().expect("missing Method header"),
            &Header::Method(Method::Connect)
        );
